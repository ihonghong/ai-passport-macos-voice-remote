...
当前 fork 会直接进入 Mac 无线语音快捷键遥控器。实体上键、下键与 OK 键均可直接配置；
默认值依次是 Command-Delete、按住说话并发送一组键盘组合键、回车。内置麦克风通过
BLE HID 传输到仓库内的 macOS Bridge；屏幕显示时间、电量、连接状态、可选模型额度、
今日 Token 和可选的编译期宠物。

实体下键按住时默认发送**左 Control + 左 Command**。这只是项目默认映射，不是所有
