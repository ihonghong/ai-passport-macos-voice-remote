...
bool shortcut_keymap_valid(const shortcut_keymap_t *keymap)
{
    ...
    for (size_t i = 0; i < SHORTCUT_ACTION_COUNT; ++i) {
        const shortcut_chord_t chord = keymap->action[i];
                            ...
                            uint8_t output[SHORTCUT_KEYMAP_WIRE_BYTES])
{
    ...
    for (size_t i = 0; i < SHORTCUT_ACTION_COUNT; ++i) {
        output[1 + i * 2] = keymap->action[i].modifiers;
        output[2 + i * 2] = keymap->action[i].key_code;
    }
                            ...
                            shortcut_keymap_t *keymap)
{
    ...
    for (size_t i = 0; i < SHORTCUT_ACTION_COUNT; ++i) {
        decoded.action[i].modifiers = data[1 + i * 2];
        decoded.action[i].key_code = data[2 + i * 2];
    }
