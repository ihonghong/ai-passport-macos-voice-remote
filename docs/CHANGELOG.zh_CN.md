...
- 新增 macOS 输入源断线保护：Passport 模式只在 Bridge 与设备正常连接时保持
  BlackHole；Bridge 等待、异常或停止时临时恢复上一次物理麦克风，重新连接后按用户
  原选择恢复，会议模式不会被自动覆盖。
- 重新调整快捷键仪表盘的字体层级：设备运行状态成为第一视觉信息，时间、电量、额度
  与今日 Token 数值统一为更克制的次级字号。READY 与 LISTENING 现在共用
  同一个 20px 居中锚点，开始录音时只更新颜色和上下文，状态文字不再跳动。
- 完成独立开源发布收尾：公开默认值改为通用文案并关闭指标与宠物，固定 Mac 依赖版本，
  加固安装器删除路径与 BLE 配对日志，新增 macOS Host CI，并将自动同步上游改为人工审查。
- 修复 macOS LaunchAgent 精简 PATH 下的 Codex Provider 自动检测：先解析受支持的
  `~/.local/bin/codex` 回退路径，再决定是否关闭指标。
- 明确每位用户都需要把所选听写应用或输入法绑定为左 Control + 左 Command，并移除
  运行日志中残留的豆包专属文案。
- 新增直达式 Mac 语音快捷键遥控：显示时间、电量、Codex 剩余量、今日 Token 进度
  与宠物聆听动画；通过 BLE HID 映射按住说话、回车与 Command-Delete；通过
  厂商自定义 HID 报告无线传输 8 kHz 麦克风音频；macOS BlackHole 桥接支持无线
  自动重连并保留 USB 回退。
- 将完整的 macOS Bridge 和状态栏应用迁入 `host/macos`，新增可重复执行的安装、卸载、
  诊断脚本，并补齐面向普通用户和安装 Agent 的从克隆到可用流程。
- 将可选模型指标提取为主机 Provider 插件，内置 `auto`、`codex`、`none`，并提供对应的
  固件编译期文案配置。
- 新增编译期宠物插件；由于所有者的宠物素材没有确认的再分发许可证，仅作为已忽略的本地
  回退继续供当前所有者使用。
- 在不改变快捷键语义的前提下降低无线空闲功耗：启用 BLE modem sleep 与自动动态
  降频；以非致命回退方式协商空闲/录音连接参数；断开后先以 30-50 ms 间隔广播
  30 秒，再退避到 500-1000 ms，实体按键可恢复快速广播；音频任务空闲时改为阻塞
  等待，不再每 10 ms 轮询。自动 Light-sleep 仍保持关闭，等待按键延迟、显示与音频
  的联合实机验证。
- 将开发样机的固定 BLE 配对码替换为每次新配对随机生成的 6 位码，并增加设备端临时
  配对浮层；加密完成或连接断开后自动隐藏。
- 新增无线显示深度空闲层：本地无操作满 1 分钟后，背光从 15% 降到 5%，暂停 UI
  动画并降低电量轮询频率；实体键或再次使用会恢复显示。
